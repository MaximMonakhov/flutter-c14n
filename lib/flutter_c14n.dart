...
class FlutterC14n {
  static const MethodChannel _channel = MethodChannel('flutter_c14n');

  static Future<String> get platformVersion async {
    final String? version = await _channel.invokeMethod<String>('getPlatformVersion');
    if(version == null) throw Exception('Произошла ошибка при выяснении версии платформы');
    return version;
  }
  ...
  static Future<String> canonicalize(
    ...
    try {
      return (await _channel.invokeMethod<String>("canonicalize", args))!;
    } catch(e) {
      throw Exception('Ошибка при канонизации: ${e.toString()}');
    }
  }
}
